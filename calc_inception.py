...
if __name__ == '__main__':
    ...
    assert args.dataset in ['compcar', 'afhq', 'ffhq', 'cats', 'celeba']
    ...
    dset = LMDBDataset(args.path, transform=transform, resolution=args.datasize)
    loader = DataLoader(dset, batch_size=args.batch, num_workers=4)

    features = extract_features(loader, inception, device)
    if features.size(0) < args.n_sample:
        args.n_sample = features.size(0)
    ...
    with open(f'inception_{args.dataset}_{args.n_sample}_{args.size}.pkl', 'wb') as f:
